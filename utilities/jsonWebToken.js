    ...
    getClaims() {
        const claims = {
            ...
            application: {
                ...
                accountType:this.payload.accountType,
                currency:this.payload.defaultCurrency || null
            },
        };
        return claims;
    }
    ...
    static async verifyToken(req, res, next) {
        ...
        if (token) {
            try {
                ...
                if (e.name === 'TokenExpiredError') return res.status(401).json({error:'User token expired'});
                if (e.message === 'jwt malformed') return res.status(400).json({error:'Invalid Jwt format'});
                return res.status(400).json({error:'Authorization error'});
            }
        } else {
                return res.status(400).json({error:"Authorization token not present"});
        }
    }
}
